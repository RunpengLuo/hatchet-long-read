...
def run(args=None):
    ...
    if eval_all and result_dir is not None:
        ...
        out_path = os.path.join(out_dir, const.POOL_EVAL_TSV)
    ...
    if seg_file is None:
        ...
        seg_file = const.BEST_SEG_UCN(result_dir)
    ...
    if all_results:
        ...
        out_snv = os.path.join(out_dir, const.SOMATIC_SNVS_TSV)
        ...
        if genome_size is not None:
            ...
            for sample in samples:
                ...
                out_plot = const.VAF_1D_PDF(out_dir, sample)
                plot_vaf_1d(sample_df, segs_plot, genome_axis, out_plot)

    if summary_rows:
        summary_df = pd.DataFrame(summary_rows)
        out_summary = os.path.join(out_dir, const.EVAL_SUMMARY_TSV)
