...
def parse_phase_snps_arguments(args=None):
    ...
    # check required panel reference exists
    if config.urls.refpanel_genome_chr_notation:
        panel_reffile = os.path.join(rpd, f"{panel_refver}_chr.fa")
    else:
        panel_reffile = os.path.join(rpd, f"{panel_refver}_no_chr.fa")
    ensure(
        isfile(panel_reffile),
        f"{panel_reffile} is required for phase_snps, but it does not exist!",
    )
    ver = args.refversion
    chrnot = args.chrnotation
    # check required rename_chrs*.txt file, generated by download_panel.py
    # will removed in the future
    rename_files = None
    if ver == panel_refver and chrnot:
        rename_files = [
            os.path.join(rpd, "rename_chrs1.txt"),
            os.path.join(rpd, "rename_chrs2.txt"),
        ]
        for f in rename_files:
            ensure(
                isfile(f),
                f"{f} is required for phase_snps, but it does not exist!",
            )

    # check all required chain files if liftover is required
    chains = None
    if ver != panel_refver:
        if chrnot:
            chains = {
                f"{ver}_{panel_refver}": os.path.join(
                    rpd, f"{ver}_{panel_refver}.chr.chain"
                ),
                f"{panel_refver}_{ver}": os.path.join(
                    rpd, f"{panel_refver}_{ver}.chr.chain"
                ),
            }
        else:
            chains = {
                f"{ver}_{panel_refver}": os.path.join(
                    rpd, f"{ver}_{panel_refver}.no_chr.chain"
                ),
                f"{panel_refver}_{ver}": os.path.join(
                    rpd, f"{panel_refver}_{ver}.no_chr.chain"
                ),
            }
        for chain_file in chains.values():
            ensure(
                isfile(chain_file),
                f"{chain_file} is required for liftover, but it does not exist!",
            )
    ...
    return {
        ...
        "refvers": ver,
        "chrnot": chrnot,
        "panel_genome": panel_reffile,
        "panel_refvers": panel_refver,
        "chains": chains,
