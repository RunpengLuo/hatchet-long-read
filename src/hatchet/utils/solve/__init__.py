...
def solve(
    ...
    if not binwise:
        ...
        with open(f"{tempdir}/config.txt", "w") as fd:
            ...
            fd.write(f"d={d}\n")
            fd.write(f"penalty_term={problem_params}\n")
            fd.write("========================================\n")
        ...
        for i0 in range(0, num_steps + 1):
            param = step_size * i0
            tempdir_ = f"{tempdir}/instances/solve_{pname}_{param}"
        ...
        return model_selection(f_a, f_b, weights, instances, pname, tempdir)
    else:
...
def model_selection(f_a, f_b, weights, instances, pname, tempdir):
    """
    ...
    for param, [tobj, cA, cB, u, _, _] in sorted(
        ...
        [imf_obj, reg_obj] = compute_individual_objs(pname, weights, f_a, f_b, cA, cB, u)
    ...
    plt.savefig(f"{tempdir}/pareto_curve.{pname}.png", dpi=300)
