...
def main(args=None):
    ...
    log(msg="# Reading the combined BB file\n", level="STEP")
    ...
    log(msg="# Checking consistency of results\n", level="STEP")
...
def read_bb(bbfile, subset=None):
    ...
    for ch, df0 in bb.groupby("#CHR"):
        ...
        for sample, df in df0.groupby("SAMPLE"):
            ...
            if len(gaps) > 0:
                if len(gaps) > 1:
                    log("Found a chromosome with >1 gaps between bins. Taking the largest"
